...
Arr::toPostgresArray([null, 'x']);         // '{null,x}'     ← строка 'null', а не NULL
```
...
**Влияние.** Тихая порча данных при записи в PostgreSQL. Значения с запятой разбиваются на несколько элементов; `null`
записывается как строка `'null'`; строки с кавычками ломают литерал. Ошибка не проявляется до чтения данных обратно.
